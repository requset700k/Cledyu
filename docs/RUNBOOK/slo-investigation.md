...
| 영역 | SLO | SLI 소스 |
|---|---|---|
| API 요청 가용성 | 99.5% | `http_requests_total{path=~"/api/v1/.*", path!="/api/v1/sessions/:id/ws", status=~"5.."}` / `http_requests_total{path=~"/api/v1/.*", path!="/api/v1/sessions/:id/ws"}` |
| API 요청 지연 | 99.5% 요청 1초 이내 | `http_request_duration_seconds_bucket{path=~"/api/v1/.*", path!="/api/v1/sessions/:id/ws", le="1"}` |
...
curl -G -s http://127.0.0.1:9090/api/v1/query \
  --data-urlencode 'query=sum(rate(http_requests_total{path=~"/api/v1/.*",path!="/api/v1/sessions/:id/ws",status=~"5.."}[30m])) / sum(rate(http_requests_total{path=~"/api/v1/.*",path!="/api/v1/sessions/:id/ws"}[30m]))' \
  | jq '.data.result'
```
