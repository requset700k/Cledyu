...
func (e *KubeVirtExecutor) Exec(ctx context.Context, cmd string) (string, error) {
	...
	// --known-hosts "": 빈 문자열로 host key 검증을 끈다. /dev/null은 빈 known_hosts로
	// 취급되어 새 VM 접속 시 "yes/no?" 프롬프트가 뜨고, 비대화형 환경에서 execTimeout까지 block된다.
	args := []string{"ssh", "--username", e.sshUser, "--known-hosts", ""}
	if e.sshKey != "" {
