...
const LINES: { text: string; color?: string; delay: number; duration: number; steps: number }[] = [
  { text: '$ cledyu start lab/docker-nginx', delay: 0.4, duration: 0.9, steps: 32 },
  { text: '⠿ provisioning vm (kubevirt) … 42s', color: 'text-white/45', delay: 1.6, duration: 0.7, steps: 30 },
  { text: '✔ session ready — 3h remaining', color: 'text-white/80', delay: 2.6, duration: 0.6, steps: 28 },
  ...
  { text: '✔ STEP 3/4 passed · +50 pts', color: 'text-white/80', delay: 7.3, duration: 0.6, steps: 28 },
];
          ...
          <div key={i} className="overflow-hidden">
            ...
            {i === LINES.length - 1 && (
              <span
                ...
                style={{ animation: 'cledyu-blink 1s step-end infinite', animationDelay: `${line.delay}s` }}
              />
            )}
          </div>
        ))}
