...
// TestResultUnmarshal은 검증엔진(validation-results)이 내보내는 JSON 형식을 그대로
// validation.Result로 역직렬화할 수 있는지(필드 태그 일치) 검증한다 — 엔진 model.ValidationResult와의
// 스키마 드리프트를 잡는 가드.
func TestResultUnmarshal(t *testing.T) {
	// 검증엔진 model.ValidationResult 직렬화 형태와 동일한 페이로드.
	payload := `{
		"trace_id": "abc123",
		"session_id": "sess-1",
		"step_id": 3,
	...
	if r.Checks[0].Type != "command" || r.Checks[0].Passed || r.Checks[0].Detail == "" {
		t.Errorf("check[0] wrong: %+v", r.Checks[0])
	}
	if r.Checks[1].Type != "file_exists" || !r.Checks[1].Passed {
