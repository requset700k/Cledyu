...
// SetVMFiles는 세션 VM 파일 조회 경로를 주입한다. Secret/RBAC 미설정 환경에서도 API 기동은
// 유지해야 하므로 생성자 필수 인자가 아니라 선택적 setter로 둔다.
func (h *Handler) SetVMFiles(service vmFileService) { h.vmFiles = service }
