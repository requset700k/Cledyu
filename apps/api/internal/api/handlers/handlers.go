...
// Dispatcher는 검증 요청을 검증엔진(Kafka)으로 발행하는 의존성이다.
// 실제로는 *validation.Producer가, 테스트에서는 fake가 주입된다. nil이면 발행을 건너뛴다.
type Dispatcher interface {
	Publish(ctx context.Context, req validation.Request) error
}
...
type Handler struct {
	...
	steps    *stepStore                    // 세션별 스텝 진행 상태 in-memory(검증 결과 소비 연동 전까지 유지).
	virt     kubecli.KubevirtClient        // VM serial console 접속용 KubeVirt 클라이언트. nil이면 콘솔 비활성.
	dispatch Dispatcher                    // 검증 요청 발행. nil 허용 — Kafka 미설정 시 발행 생략.
}
...
func New(cfg *config.Config, log *zap.Logger, sessions *kubevirt.Manager, dispatch Dispatcher) *Handler {
