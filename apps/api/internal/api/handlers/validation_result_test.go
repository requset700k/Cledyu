...
// passed 결과는 스텝을 passed로 확정하고 다음 스텝을 활성화하며 체크별 상세를 저장한다.
func TestApplyValidationResult_Pass(t *testing.T) {
	h := resultTestHandler()
	h.steps.m["s1"] = twoStepSession("validating")

	h.ApplyValidationResult(validation.ValidationResult{
		SessionID: "s1", StepID: 1, Passed: true,
		Checks: []validation.CheckResult{{Type: validation.CheckCommand, Passed: true, Detail: "ok"}},
	})
...
// failed 결과는 스텝을 failed로 두되 진행시키지 않고(current 유지) 실패 사유(detail)를 저장한다.
func TestApplyValidationResult_Fail(t *testing.T) {
	...
	h.ApplyValidationResult(validation.ValidationResult{
		...
		Checks: []validation.CheckResult{{Type: validation.CheckCommand, Passed: false, Detail: "expected 600 got 644"}},
	})
...
// 모르는 세션/스텝의 결과(지연 도착 등)는 무시한다.
func TestApplyValidationResult_UnknownSessionOrStep(t *testing.T) {
	h := resultTestHandler()
	h.ApplyValidationResult(validation.ValidationResult{SessionID: "nope", StepID: 1, Passed: true}) // panic 없어야 함

	h.steps.m["s1"] = &sessionSteps{Steps: []stepState{{StepID: 1, Status: "active"}}, CurrentStep: 1}
	h.ApplyValidationResult(validation.ValidationResult{SessionID: "s1", StepID: 99, Passed: true})
